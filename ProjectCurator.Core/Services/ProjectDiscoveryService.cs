...
public class ProjectDiscoveryService
{
    ...
    public ProjectDiscoveryService(ConfigService configService, IShellService shellService)
    {
    ...
    private void SetupExternalSharedPaths(string docRoot, string boxShared, string[] newPaths, List<string> logs)
    {
        ...
        foreach (var savedPath in pathsToProcess)
        {
            ...
            {
                if (_shellService.IsSymlink(linkPath))
            ...
            {
                _shellService.CreateSymlink(linkPath, expandedPath);
    ...
    private void RemoveJunctionInternal(string path)
    {
        ...
        if (_shellService.IsSymlink(path))
        {
    ...
    private void CreateJunctionInternal(string linkPath, string targetPath)
    {
        ...
        _shellService.CreateSymlink(linkPath, targetPath);
    }
    ...
    private void CheckJunction(ProjectCheckResult result, string name, string path, bool mandatory)
    {
    ...
    private string GetJunctionStatus(string path)
    {
        var info = new DirectoryInfo(path);
        if (!info.Exists) return "Missing";
        if (_shellService.IsSymlink(path))
        {
            var target = _shellService.ResolveSymlinkTarget(path);
    ...
    private ProjectInfo BuildProjectInfo(string name, string path, string tier, string category, DateTime now)
    {
