...
public class ContextCompressionLayerService
{
    ...
    public ContextCompressionLayerService(ConfigService configService, IShellService shellService)
    {
        ...
        CancellationToken ct = default)
    {
        return await Task.Run(() =>
        {
            ...
            try
            {
                ...
                var projectSetupOk = SetupProject(projectName, tier, category, localRoot, syncRoot, obsidianRoot, force, resetSkills, result.Logs, _shellService);
                if (!projectSetupOk)
                {
        ...
        IShellService shellService)
    {
        ...
        EnsureJunction(contextJunction, obsidianAiContext, logs, shellService);
        ...
        SetupCliSkillsJunctions(projectRoot, boxProjectRoot, force, logs, shellService);
        return true;
    }
    ...
    private static void SetupCliSkillsJunctions(string localProjectRoot, string boxProjectRoot, bool force, List<string> logs, IShellService shellService)
    {
        foreach (var cli in new[] { ".claude", ".codex", ".gemini" })
        {
            ...
            logs.Add($"[DEBUG] {cli}: localExists={localEntryExists} force={force}");

            if (localEntryExists)
            {
                if (shellService.IsSymlink(localPath))
                {
                    // symlink/junction: ターゲットが正しいか検証
                    string? actualFull = null;
                    try
                    {
                        var target = shellService.ResolveSymlinkTarget(localPath);
                        actualFull = target != null ? Path.GetFullPath(target) : null;
                    }
                    catch (Exception ex)
                    {
                        Debug.WriteLine($"[SetupCliSkillsJunctions] ResolveSymlinkTarget failed: {ex.Message}");
                    }
            ...
            EnsureJunction(localPath, boxPath, logs, shellService);
        }
    }
...
""";
    ...
    private static void EnsureJunction(string linkPath, string targetPath, List<string> logs, IShellService shellService)
    {
        ...
        if (linkEntryExists)
        {
            if (shellService.IsSymlink(linkPath))
            {
                try
                {
                    var resolved = shellService.ResolveSymlinkTarget(linkPath);
                    var expectedFull = Path.GetFullPath(targetPath);
                    var actualFull = resolved != null ? Path.GetFullPath(resolved) : null;
                    if (actualFull == expectedFull)
                    {
                ...
                {
                    Debug.WriteLine($"[EnsureJunction] ResolveSymlinkTarget/Delete failed: {ex}");
        ...
        try
        {
            shellService.CreateSymlink(linkPath, targetPath);
    ...
    public const string ProjectSummaryTemplate = """
# Project Summary
...
""";
    ...
    public const string CurrentFocusTemplate = """
# Focus
