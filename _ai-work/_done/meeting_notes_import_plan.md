...
[Apply: チェックされた項目だけを適用]
  ...
  - Tensions: tensions.md のセクションに追記 (ファイル不在時は新規作成)
        │
        ▼
...
MeetingNotesService (Services/MeetingNotesService.cs)
  ...
  ├── ApplyTensionsAsync()     tensions.md のセクション別追記
...
|---|---|---|
| tensions.md パス解決 | `CaptureService` の同様ロジック | 同じロジックを独自実装 (workstream フォールバックあり) |
| tensions.md 末尾への1行追記 | `CaptureService.AppendToTensionsAsync()` | セクション別挿入が必要なため別実装 |
...
2. current_focus.md と tensions.md を読み込み (不在なら空文字)
...
│  └─ Tab "Tensions (N)"
│      ScrollViewer > StackPanel:
│        [x] "Add to tensions.md"  (HasItems=false なら IsEnabled=false)
│        AvalonEdit (readonly): AppendContent を表示
│        tensions.md 不在時: "tensions.md not found — will be created"
│
...
| ファイル | 参照用途 |
|---|---|
| `Services/CaptureService.cs` | tensions.md パス解決・新規作成テンプレートのパターン |
