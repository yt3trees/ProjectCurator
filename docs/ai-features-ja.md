...
「+ Import Source」をクリックするか、Wiki タブにドラッグ＆ドロップします。LLM が以下を自動実行します:
...
対応形式: `.md` / `.txt` (PDF / Word は現在テキスト変換が必要)

LLM レスポンスは JSON 形式で受け取り、作成・更新されたページ数をステータスバーに表示します。

#### Import のプロンプト構成

LLM 呼び出しは 1 回の `ChatCompletionAsync` (system + user) で完結します。
...
- 100 ページ以下 (Small モード): 全ページ内容 (最大 50 件) をそのまま LLM に渡す
- 100 ページ超 (Medium+ モード): まず関連ページを 5 件選ばせてから、そのページ内容で回答を生成 (2 回の LLM 呼び出し)

「Save as Wiki Page」で回答を `pages/analysis/` に保存できます。

#### Query のプロンプト構成

Small モードは 1 回の `ChatCompletionAsync`、Medium+ モードは 2 回になります。
...
[Small モード] ユーザープロンプトに含まれる情報:
- 質問文
- `index.md` の全文
- 関連ページの全文 (最大 50 件)

[Medium+ モード] 1 回目 (ページ選択):
- システム: "wiki search assistant。ファイルパスのみ 1 行ずつ返せ"
- ユーザー: 質問文 + `index.md` 全文 → 関連ページ 5 件のパスを返させる

[Medium+ モード] 2 回目: Small モードと同じ構成で、選択した 5 件のページ内容を渡す
