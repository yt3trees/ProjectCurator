...
Import によって生成されるページはカテゴリに分類されます。LLM が Import 時に内容に基づいて自動で分類します。

以下の 4 カテゴリは組み込みで、削除できません:
...
カスタムカテゴリは Prompts タブから追加・削除できます。カテゴリ定義は wiki ドメインフォルダ内の `.wiki-categories.json` に保存されます。wiki ルートの `AGENTS.md` は現在のカテゴリ一覧を反映して自動更新されます。
...
Prompts タブから Import・Query・Lint それぞれの組み込みシステムプロンプトを拡張できます。プロンプト全体を置き換えるのではなく、前後に追記する形で動作します。

各操作で設定できるフィールド:

| フィールド | 効果 |
|---|---|
| System Prefix | 組み込みシステムプロンプトの前に挿入 |
| System Suffix | 組み込みシステムプロンプトの後に追加 |

2 つのフィールドの間に、各操作のデフォルトシステムプロンプトが読み取り専用で表示されます。合成後のプロンプト全体を確認しながら設定できます。

カスタムプロンプト設定は wiki ドメインフォルダ内の `.wiki-prompts.json` に保存されます。「Reset to Defaults」ボタンで全ての Prefix・Suffix フィールドを空に戻せます (組み込みプロンプト自体は変更されません)。

wiki-schema.md もこのタブから直接編集できます。変更は次回の Import または Query 実行時に反映されます。未保存の変更がある場合は「Save Schema」ボタンの横にインジケータが表示されます。

カテゴリ管理もこのタブから行えます。カスタムカテゴリの追加・削除が可能です。組み込みの 4 カテゴリ (sources, entities, concepts, analysis) は削除できません。
