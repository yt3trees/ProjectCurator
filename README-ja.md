...
- Editor のツールバーに `Update Focus from Asana` ボタンが表示される
...
6. AI機能が有効な場合は `Update Focus from Asana` ボタンでLLMによる更新提案を取得
...
flowchart TD
    ...
    E --> F["Asana Sync実行(任意)"]
    F --> G["Update Focus from Asana(任意、AI)"]
```
...
| Editor | コンテキスト用Markdown編集、検索、リンクジャンプ、decision_log追加、AI機能有効時は "Update Focus from Asana" ボタンで自動更新提案 |
